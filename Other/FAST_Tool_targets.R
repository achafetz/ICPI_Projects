...
##   A.Chafetz, USAID
##   Purpose: targets to populate FAST Tool
##   Date: Dec 22, 2017
...
#subset
  df_targets <- df_targets %>%
    ...
    filter(indicator %in% c("TX_NEW", "TX_CURR", "HTS_TST", "HTS_TST_POS", "PMTCT_STAT", "PMTCT_ART", "OVC_SERV", "VMMC_CIRC"),
           disaggregate == "Total Numerator") %>% 
    #summarize at prioritization/im level
    group_by(operatingunit, fy17snuprioritization, primepartner, mechanismid, implementingmechanismname, indicator) %>% 
